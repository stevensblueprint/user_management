...
func TestAddUserHandler_UserAlreadyExists(t *testing.T) {
	// Create a temporary users.yaml file for testing
	tempFile, err := os.CreateTemp("", "users.yaml")
	if err != nil {
		t.Fatalf("Failed to create temporary file: %v", err)
...
`
	...
	// Create a test request JSON with an existing user
	userReq := UserRequest{
...
func TestAddUserHandler_MissingFields(t *testing.T) {
	tempFile, err := os.CreateTemp("", "users.yaml")
	if err != nil {
		t.Fatalf("Failed to create temporary file: %v", err)
	}
...
`
	...
	// Create a test request JSON with missing fields
	userReq := UserRequest{
		Username: "newuser",
		// Missing password
		Displayname: "New User",
